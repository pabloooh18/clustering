...
for cluster in cluster_ids:
	...
	for document in os.listdir(cluster_folder):	
		...
		if var==0:
			...
			for_printf=""
			#summary_limit limite por byte 665 ~100 palabras
			#summary_wordlimit por palabra
			for_printf=" ".join(summary_limit(resume_printf,summary_sentence_scores,665))
			...
			# except:
			# 	pass
			# with open("%s/%s_%s.txt" % (folder_path, document, count_printf), "w") as f: f.write(for_printf)

			print("\n\n\nResumen de "+str(count_printf)+"--" + document +": " +for_printf)
			count_printf+=1	
			if count_printf==9:
				#import ipdb;ipdb.set_trace()
				contains_words1=[]
